...
function isLikelyEthAddress(input) {
    return /^0x[a-fA-F0-9]{40}$/.test(input.trim());
}
function toRangeKey(v) {
    if (v === "7d" || v === "30d" || v === "90d")
        return v;
    return "30d";
}
router.get("/analyze", (req, res) => {
    ...
    if (chain !== "eth")
        return res.status(400).json({ error: "Only chain=eth is supported in MVP." });
    if (!isLikelyEthAddress(address))
        return res.status(400).json({ error: "Invalid address. Must be a 0x address with 40 hex chars." });
